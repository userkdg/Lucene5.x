...
public class IndexCreator implements Runnable {
	...
	//"C:/doc" + (i+1), "C:/lucenedir" + (i+1)
	public IndexCreator(String docPath, String luceneDir,int threadCount,CountDownLatch countDownLatch1,CountDownLatch countDownLatch2) throws IOException {
		super();
		if(Files.notExists(Paths.get(docPath))) {
			Files.createDirectory(Paths.get(docPath));
		}
		if(Files.notExists(Paths.get(luceneDir))){
			Files.createDirectory(Paths.get(luceneDir));
		}
